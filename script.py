...
# --- НАСТРОЙКИ (пожалуйста, проверьте и измените при необходимости) ---
FLEX_EXE = r"C:\Program Files\FlexPDE8\FlexPDE8n.exe"   # Укажите свой путь!
TEMPLATE_PDE = "task2.pde"          # Имя вашего файла с моделью
NUM_POINTS = 16                        # Должно совпадать с numP в .pde
NUM_RUNS = 9990                         # Количество прогонов 
DB_NAME = "anisotropy_results.db"
...
def main():
    ...
    for run_idx in range(1, NUM_RUNS+1):
        ...
        if data is None:
            ...
            continue
        # Проверяем, что считанные Aij совпадают с переданными (допуск)
        tol = 1e-6
        if (abs(data['A12'] - A12) > tol or abs(data['A16'] - A16) > tol or
            abs(data['A22'] - A22) > tol or abs(data['A26'] - A26) > tol or
            abs(data['A66'] - A66) > tol):
            print(f"  Несовпадение Aij: передано ({A12:.4f},{A16:.4f},{A22:.4f},{A26:.4f},{A66:.4f}) "
                  f"получено ({data['A12']:.4f},{data['A16']:.4f},{data['A22']:.4f},{data['A26']:.4f},{data['A66']:.4f})")
            # Всё равно сохраняем? Можно сохранить как есть.
        # Сохраняем в БД
